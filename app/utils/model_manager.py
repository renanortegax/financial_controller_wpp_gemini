...
class ModelManager:
    ...
    def _block_model(self, model, error_str: str, notification_callback=None):
        ...
        if notification_callback:
            notification_callback(model.model_name, unblock_at)

    def _extract_retry_delay_error(self, error_str: str) -> int | None:
        match = re.search(r'seconds:\s*(\d+)', error_str)
        return int(match.group(1)) if match else None
    
    def generate_content(self, prompt, notification_callback=None, **kwargs):
        ...
        for model in self.models:
            ...
            try:
                ...
                if "429" in error_str or "ResourceExhausted" in error_str:
                    self._block_model(model, error_str, notification_callback)
                else:
